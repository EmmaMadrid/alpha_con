...
const handlePagarClick2 = async () => {
  const stripe = await loadStripe('pk_test_51P3tpSCExtLBu1XZ5cBhS37icvXqLIIrW9ZdJYLWcumHCd5MGnUmdxXpp8U07z7ODP8GFFqXl1NyRl2pf3eqUFA600hn1IbqP8');

  if (stripe) {
    const { error } = await stripe.redirectToCheckout({
      lineItems: [{ price: 'price_1POnPCCExtLBu1XZEhzek2fe', quantity: 1 }],
      mode: 'payment',
      successUrl: 'http://localhost:5173/PagoExitoso',
      cancelUrl: 'http://localhost:5173/PagoFallido',
    });
...
const handlePagarClick3 = async () => {
  const stripe = await loadStripe('pk_test_51P3tpSCExtLBu1XZ5cBhS37icvXqLIIrW9ZdJYLWcumHCd5MGnUmdxXpp8U07z7ODP8GFFqXl1NyRl2pf3eqUFA600hn1IbqP8');

  if (stripe) {
    const { error } = await stripe.redirectToCheckout({
      lineItems: [{ price: 'price_1POnPuCExtLBu1XZrDyDpXmn', quantity: 1 }],
      mode: 'payment',
      successUrl: 'http://localhost:5173/PagoExitoso',
      cancelUrl: 'http://localhost:5173/PagoFallido',
    });
    ...
    <div className='todo'>
      ...
      <div className='cuadro'>
        <div className='ticket'>
          ...
          <div className='precio-ticket'>$150</div>
        ...
        <div className='ticket'>
          ...
          <button className='comprar-boleto' onClick={handlePagarClick2}>Comprar</button>
        </div>
        ...
        <div className='ticket'>
          ...
          <button className='comprar-boleto' onClick={handlePagarClick3}>Comprar</button>
        </div>
      </div>
    </div>  
  ); 
